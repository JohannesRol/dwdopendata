...
def search_folder(key: str, unique: bool = True) -> list:
    """ Search the dwd_tree.txt files folder for a keyword
...
class Location:
    ...
    def wind(self, start, end, reso='10_minutes', where='cdc_obDE_climate'):
        ...
        wind = 'wind'
        wind_paths = search_folder(wind)
        for wp in wind_paths:
            if reso in wp['path'] and where in wp['path']:
                path = wp['path']
        ftp = self.ftp_login()
        ftp.cwd(path)
        # checks in which directorys the fitting data is
        time_matrix = dict()
        now = dt.now()
